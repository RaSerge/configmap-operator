...
import (
	...
	"github.com/raserge/configmap-operator/pkg/buildinfo"
	"github.com/raserge/configmap-operator/pkg/controllers"
	"github.com/raserge/configmap-operator/pkg/mzlog"
	"go.uber.org/zap"
...
func init() {
	...
	check(v1alpha1.AddToScheme(scheme), "Unable to add configmaps.scartel.dc/v1alpha1 to scheme")
...
// +kubebuilder:rbac:namespace=kube-system,groups=core,resources=configmaps,verbs=get;update,resourceNames=configmapmake-controller-leader
...
func main() {
	...
	opts := manager.Options{
		...
		LeaderElectionID:        "configmapmake-controller-leader",
...
func currentNamespace() (string, error) {
	buf, err := ioutil.ReadFile("/var/run/configs/kubernetes.io/serviceaccount/namespace")
	if err != nil {
		return "", err
	}
